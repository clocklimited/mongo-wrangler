...
export ADDON_ID=$(curl --header "Content-Type: application/json" \
  ...
  https://api.northflank.com/v1/projects/$SLUG/addons | jq -r '.data.id')

export OUTPUT=$(curl --header "Content-Type: application/json" \
  --header "Authorization: Bearer $NF_API_TOKEN" \
  https://api.northflank.com/v1/projects/$SLUG/addons/$ADDON_ID/credentials | jq -r '.data.envs.MONGO_SRV_ADMIN' | sed s#/admin#/#)
...
do
  STATUS=$(
    curl --silent -G "https://api.northflank.com/v1/projects/$SLUG/addons/$ADDON_ID" \
...
curl --header "Content-Type: application/json" \
  --header "Authorization: Bearer $NF_API_TOKEN" \
  --request DELETE \
  https://api.northflank.com/v1/projects/$SLUG/addons/${ADDON_ID}
